...
pub fn Register() -> Element {
    ...
    let mut display_name_signal = use_signal(|| "".to_string());

    let email = email_signal().clone();
    let password = password_signal().clone();
    let display_name = display_name_signal().clone();
                ...
                form {
                    ...
                    div {
                        ...
                        button {
                            ...
                            onclick: move |_| {
                                to_owned![display_name, email, password];

                                async move {
                                    let client = reqwest::Client::new();
                                    let res = client.post(format!("http://{}/user/register", env::var("BACKEND_URL").unwrap_or("localhost:3030".to_string())))
